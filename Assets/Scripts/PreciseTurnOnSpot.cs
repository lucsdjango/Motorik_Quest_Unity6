...
public class PreciseTurnOnSpot : MonoBehaviour {
	...
	void Update()
	{
		...
		{
			// calls MatchTarget when in Turn state, subsequent calls are ignored until targetTime (0.95f) is reached .
			animator.MatchTarget(Vector3.one, targetRotation, AvatarTarget.Root, new MatchTargetWeightMask(Vector3.zero, 1), animator.GetCurrentAnimatorStateInfo(0).normalizedTime, 0.95f);			
		}
